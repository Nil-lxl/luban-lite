...
//#include <linux/in6.h>   /*porting*/

#define NL_DBG(...)
#define nl_geterror(e) (e)
...
long nla_strscpy(char *dst, const struct nlattr *nla, size_t dstsize);
...
int __nla_put(struct sk_buff *skb, int attrtype, int attrlen,
	       const void *data);
...
#define nlmsg_alloc() nlmsg_new(900, 0)
#endif
...
#else
int nlmsg_multicast(struct net_device *ndev, struct sk_buff *skb,
				  u32 portid, unsigned int group, gfp_t flags);
#endif
 ...
 */
static inline int nlmsg_datalen(const struct nlmsghdr *nlh)
{
	return nlh->nlmsg_len - NLMSG_HDRLEN;
}
...
static inline int nla_put_nested(struct nl_msg *msg, int attrtype, struct nl_msg *nested)
{
	NL_DBG(2, "msg %p: attr <> %d: adding msg %p as nested attribute\n",
		msg, attrtype, nested);
#if 0
	return nla_put(msg, attrtype, nlmsg_datalen(nlmsg_hdr(nested)),
		       nlmsg_data(nlmsg_hdr(nested)));
#else
	return nla_put(msg, attrtype, nested->len, nested->data);
#endif
}
