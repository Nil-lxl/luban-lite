...
int32 hgic_blenc_tx_data(uint8 *data, int32 len)
{
	...
	while(1) {
		...
		hgic_iwpriv_send_blenc_data("hg0", &adv_info, adv_info.header_info.length + 2); // adv_header(2byte)
...
{
	if(ble_ctrl == NULL) {
		ble_ctrl = (struct ble_rx_ctrl *)MALLOC(sizeof(struct ble_rx_ctrl));
	}
