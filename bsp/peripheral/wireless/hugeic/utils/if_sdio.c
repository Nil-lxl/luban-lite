...
static unsigned char *malloc_sdio_rxbuf(unsigned int buff_len,unsigned int flags)
{
    return kmalloc(buff_len, flags);
}

static void free_sdio_rxbuf(unsigned char *rxbuf)
{
    if(rxbuf) {
        kfree(rxbuf);
    }
...
#else
extern int hgic_sdio_abort(sdio_func_t *func);
extern int hgic_sdio_read_cccr(sdio_func_t *func, u8 *pending);
extern void hgic_mmc_set_blk_size(sdio_func_t *func, unsigned int blk_size);
extern unsigned char *malloc_sdio_rxbuf(unsigned int buff_len,unsigned int flags);
extern void free_sdio_rxbuf(unsigned char *rxbuf);
#endif
    ...
    do {
        ...
        if (int_status & SDIO_INIT_STATUS_DATA_READY) {
            ...
            if (len == 0xFFFFFFFF) {
                ...
                hgic_print_hex(sdiodev->rxbuf, len > 32 ? 32 : len);
                if (ret) {
                    hgic_sdio_abort(func);
                }
            } else {
                hgic_err("rx len invaild:%d\n",len);
                hgic_sdio_abort(func);
            }
        }
...
int hgic_sdio_probe(sdio_func_t *func, const sdio_device_id_t *id)
{
    int ret = 0;
    ...
    sdiodev->rxbuf = malloc_sdio_rxbuf(max_pkt_len, GFP_KERNEL);
...
void hgic_sdio_remove(sdio_func_t *func)
{
    struct hgic_sdio *sdiodev = NULL;
...
void hgic_sdio_set_probe_hdl(hgic_probe probe)
{
    probe_hdl = probe;
}

unsigned int hgic_sdio_set_max_pkt(unsigned int max_pkt)
{
