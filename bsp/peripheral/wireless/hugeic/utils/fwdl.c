...
static int hgic_bootdl_parse_fw(struct hgic_bootdl *hg, const char *fw_name)
{
    ...
    hgic_dbg("firmware %s aes_en:%d,crc_en:%d,version:%x\r\n", fw_name,
...
int hgic_bootdl_cmd_enter(struct hgic_bootdl *hg)
{
    ...
    if (ret == 0 || ret == HG_BOOTDL_RSP_ERR_IN_FW) {
        if (ret == HG_BOOTDL_RSP_ERR_IN_FW) {
            ...
            if(resp_data & 0x1 != 0) {
                hgic_dbg("SPI flash ready!\n");
                hg->flash_rdy = 0;//temp
            } else {
                hgic_dbg("NO SPI flash!\n");
                hg->flash_rdy = 0;
            }
            return STATE_BOOT;
        }
...
static int hgic_bootdl_cmd_verify_memory(struct hgic_bootdl *hg, u32 write_addr, u32 data_len)
{
    struct sk_buff *skb  = NULL;
    struct hgic_bootdl_cmd_hdr *cmd_hdr = NULL;
    s32 ret = -1;
    s32 crc_resp = 0x5A5A5A5A;

    skb = hgic_bootdl_alloc_cmd_skb(hg, HG_BOOTDL_CMD_VERIFY_MEM);
    if (skb) {
        cmd_hdr = (struct hgic_bootdl_cmd_hdr *)skb->data;
        put_unaligned_le32(write_addr, cmd_hdr->addr);
        put_unaligned_le32(data_len, cmd_hdr->len);
    ...
    ret = hgic_bootdl_send_cmd_tmo(hg, skb, HG_BOOTDL_CMD_NORMAL_TMO, &crc_resp);
    if(ret == 0 && (crc_resp == 0 || crc_resp == 0xFFFFFFFF)) {
        hgic_dbg("Verify memory success!\n");
    } else {
        hgic_err("Verify memory failed!ret:%d,crc:0x%x\n",ret,crc_resp);
        return -EFAULT;
    }
    return 0;
}
...
static int hgic_bootdl_cmd_reset(struct hgic_bootdl *hg)
{
    struct sk_buff *skb = NULL;
    struct hgic_bootdl_cmd_hdr *cmd_hdr = NULL;
    int ret = 0;
    ...
    skb = hgic_bootdl_alloc_cmd_skb(hg, HG_BOOTDL_CMD_CHIP_RESET);
    if (skb) {
        cmd_hdr = (struct hgic_bootdl_cmd_hdr *)skb->data;
        memcpy((void *)cmd_hdr->addr, (void *)BOOT_CMD_KEY, BOOT_CMD_KEY_SIZE);
        cmd_hdr->check = hgic_bootdl_cmd_check_val(hg->checksum_mode, (u8 *)&cmd_hdr->cmd, 11);
...
int hgic_bootdl_download(struct hgic_bootdl *hg, const char *fw_path)
{
    ...
    if(hg->flash_rdy) {
        hgic_dbg("Flash ready,boot download with firmware header...\n");
        data = (char *)hg->fw->data;
    } else {
    ...
    if(hg->flash_rdy) {
        ret = hgic_bootdl_cmd_reset(hg);
    } else {
        ret = hgic_bootdl_cmd_verify_memory(hg, hg->fw_info.write_addr, hg->fw_info.fw_len);
        if(ret) {
            hgic_err("Verify memory error:%d\r\n", __FUNCTION__,ret);
            goto __finish;
        } else {
            hgic_dbg("Verify memory success!\n");
        }
