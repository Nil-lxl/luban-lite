...
#if 0//ndef CONFIG_NATIVE_WINDOWS //porting
#include <grp.h>
...
//porting
    ...
    strcpy(conf->last_bss->iface, fname);
    
    rbuf = os_malloc(CONFIG_BUFF_SIZE);
    if(rbuf == NULL){
        os_free(buf);
    ...
    umac_config_read(fname, rbuf, CONFIG_BUFF_SIZE);
	while ((len = wpa_get_line(rbuf+roff, buf, 512))) {
		struct hostapd_bss_config *bss;
		...
		if (pos == NULL) {
			wpa_printf(MSG_ERROR, "Line %d: invalid line '%s'",
				   line, buf);            
            PRINTF("%s,%d,Line %d: invalid line '%s'!\n\n", __FUNCTION__, __LINE__,line, buf);
			errors++;
			continue;
		}
		*pos = '\0';
