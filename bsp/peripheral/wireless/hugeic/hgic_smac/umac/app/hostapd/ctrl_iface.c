...
//#include "common/ctrl_iface_common.h" //porting
#ifdef CONFIG_DPP
...
void hapd_cli_reply(struct hapd_interfaces *interfaces, char *reply, int reply_len); //porting
...
static int hostapd_ctrl_iface_wps_config(struct hostapd_data *hapd, char *txt)
{
    ...
    PRINTF("%s,%d:Enter,cmd:%s\n",__FUNCTION__,__LINE__,txt);
    wpa_printf(MSG_INFO,"%s:Enter,cmd:%s\n",__FUNCTION__,txt);
	...
	*pos++ = '\0';
    ...
    PRINTF("%s:set ap ssid:%s,auth:%s,encr:%s,key:%s\n",__FUNCTION__,
        ssid,auth,encr,key);
	ret = hostapd_wps_config_ap(hapd, ssid, auth, encr, key);
    if(ret) {        
        PRINTF("%s,%d:Error,cmd:%s\n",__FUNCTION__,__LINE__,txt);
    }
}
	...
	*value++ = '\0';
	...
	if (0) {
		...
		if (os_strcasecmp(cmd, "deny_mac_file") == 0) {
			//hostapd_disassoc_deny_mac(hapd);   //porting
		} else if (os_strcasecmp(cmd, "accept_mac_file") == 0) {
			//hostapd_disassoc_accept_mac(hapd);   //porting
						     ...
						     char *cmd)
{
	...
	hapd_supplicant_event(hapd, EVENT_TX_STATUS, &event);  //porting
					      ...
					      char *cmd)
{
	...
	hapd->ext_mgmt_frame_handling = 0;
	hapd_supplicant_event(hapd, EVENT_RX_MGMT, &event);  //porting
	hapd->ext_mgmt_frame_handling = 1;
					      ...
					      int reply_size)  //porting
{
	...
	if (os_strcmp(buf, "PING") == 0) {
        ...
        PRINTF("%s,%d:Enter,Match wps_config:%s\n",__FUNCTION__,__LINE__,buf);    
		...
		reply_len = hostapd_ctrl_iface_driver_cmd(hapd, buf + 7, reply,
							  reply_size);
#endif /* ANDROID */
	} else if (os_strncmp(buf, "UPDATE_CONF", 11) == 0) { //porting
        if(hostapd_reload_config(hapd->iface) < 0) 
            reply_len = -1;
    } else {
		os_memcpy(reply, "UNKNOWN COMMAND\n", 16);
        PRINTF("%s,%d:unknow cmd:%s\n",__FUNCTION__,__LINE__,buf);
		reply_len = 16;
	}
...
//porting
#if 0 //ndef CONFIG_CTRL_IFACE_UDP 
					    ...
					    int reply_size)  //porting
{
	...
	if (hapd == NULL) {
		int res = os_snprintf(reply, reply_size, "FAIL-NO-IFNAME-MATCH\n");
					      ...
					      void *sock_ctx)
{
    ...
    msg = nl_socket_recv(interfaces->global_ctrl_sock, 100);
    if (msg == NULL) {
        wpa_printf(MSG_ERROR, "rx ctrl msg failed!\r\n");
        PRINTF("%s:rx ctrl msg failed!\r\n",__FUNCTION__);
        return;
    }
    buf = (char *)msg->data;
	buf[msg->len] = '\0';
	...
	reply_len = hostapd_global_ctrl_iface_ifname(interfaces, buf, reply, reply_size);
    hapd_cli_reply(interfaces, reply, reply_len);
    os_free(reply);
    kfree_skb(msg);
#else
	struct hapd_interfaces *interfaces = eloop_ctx;
...
#if 0 //ndef CONFIG_CTRL_IFACE_UDP       //porting
...
int hostapd_global_ctrl_iface_init(struct hapd_interfaces *interface)
{
	...
	interface->global_ctrl_sock  = (int)nl_socket_alloc(NULL);
    ASSERT(interface->global_ctrl_sock);
...
fail:
	if (interface->global_ctrl_sock) {
        nl_socket_free((struct nl_sock *)interface->global_ctrl_sock);
        interface->global_ctrl_sock = 0;
    }
	return -1;
#else
#ifdef CONFIG_CTRL_IFACE_UDP
...
void hostapd_global_ctrl_iface_deinit(struct hapd_interfaces *interfaces)
{
	...
	if (interfaces->global_ctrl_sock){
        eloop_unregister_read_sock(interfaces->global_ctrl_sock);
		nl_socket_free((struct nl_sock *)interfaces->global_ctrl_sock);
        interfaces->global_ctrl_sock = 0;
    }
#else
#ifndef CONFIG_CTRL_IFACE_UDP
	char *fname = NULL;
