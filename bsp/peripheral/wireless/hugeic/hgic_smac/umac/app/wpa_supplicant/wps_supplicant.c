		       ...
		       int p2p_group, int multi_ap_backhaul_sta)
{
	...
	if (ssid == NULL) {
        PRINTF("%s,%d:wpas_wps_add_network failed!\n",__FUNCTION__,__LINE__);
        return -1;
    }
	ssid->temporary = 1;
...
static u16 wps_fix_config_methods(u16 config_methods)
{
	...
	if ((config_methods &
        ...
        PRINTF("WPS: Converting push_button to "
			   "virtual_push_button for WPS 2.0 compliance\n");
			      ...
			      struct wps_context *wps)
{
    ...
    PRINTF("%s:WPS: UUID %s: %s\n", __FUNCTION__,src, buf);
}
			    ...
			    struct wpa_ssid *ssid, struct wpa_bss *bss)
{
	...
	if (eap_is_wps_pbc_enrollee(&ssid->eap)) {
        ...
        PRINTF("   selected based on WPS IE "
			   "(Active PBC)\n");
		wpabuf_free(wps_ie);
		return 1;
	}
			      ...
			      struct wpa_bss *selected, struct wpa_ssid *ssid)
{
    ...
    PRINTF("%s: Check whether PBC session overlap is "
		   "present in scan results; selected BSSID "MACSTR"\n",__FUNCTION__,
		   MAC2STR(selected->bssid));
