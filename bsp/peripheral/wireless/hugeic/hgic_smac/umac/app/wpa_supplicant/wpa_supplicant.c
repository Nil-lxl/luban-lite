...
#include "utils/includes.h"//porting
...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_msg(wpa_s, MSG_INFO, "%s:Authentication with " MACSTR " timed out.",__FUNCTION__,
		MAC2STR(bssid));
	 ...
	 */
	wpa_supplicant_req_scan(wpa_s, 0, 5000);//porting
}
				       ...
				       struct wpa_ssid *ssid)
{
    ...
    PRINTF("%s,%d:Enter,check key_mgmt:%d\n",__FUNCTION__,__LINE__,wpa_s->key_mgmt);
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "%s,%d:WPA: Selected cipher suites: group %d "
		"pairwise %d key_mgmt %d proto %d",__FUNCTION__,__LINE__,
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (wpa_s->connect_without_scan ||
		...
		wpa_supplicant_req_scan(wpa_s, 0, disconnected ? 5000 : 0);//porting
	}
...
static int select_driver(struct wpa_supplicant *wpa_s, int i)
{
	...
	if (wpa_drivers[i]->global_init && global->drv_priv[i] == NULL) {
		global->drv_priv[i] = wpa_drivers[i]->global_init(global, wpa_s->ifname);//porting
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
    ...
    PRINTF("%s:RSN: flushing PMKID list in the driver\n",__FUNCTION__);
	...
	if (wpa_supplicant_enabled_networks(wpa_s)) {
		...
		if (!wpa_s->p2p_mgmt &&
		    wpa_supplicant_delayed_sched_scan(wpa_s,
						      interface_count % 3,
						      100000))
			wpa_supplicant_req_scan(wpa_s, 0, 0);//porting
#endif /* ANDROID */
		interface_count++;
	} else
...
/*
    PRINTF("%s:Starting radio work '%s'@%p after %ld.%06ld second wait\n",
		__FUNCTION__,work->type, work, diff.sec, diff.usec);
  */      
	work->started = 1;
	work->time = now;
				     ...
				     const struct wpa_interface *iface)
{
    ...
    if(iface->confname) {
        ...
        PRINTF("%s:No confname,alloc empty config...\n",__FUNCTION__);
        wpa_s->conf = wpa_config_alloc_empty(iface->ctrl_interface,
						     iface->driver_param);
    } 
#endif
	...
	if (wpa_s->conf == NULL) {
        ...
        PRINTF("%s,%d:Error,confname %s\n",__FUNCTION__,__LINE__,iface->confname);
		return -1;
	}
	...
	if (os_strlen(iface->ifname) >= sizeof(wpa_s->ifname)) {
		wpa_printf(MSG_ERROR, "\nToo long interface name '%s'.",
			   iface->ifname);
        PRINTF("%s,%d:Error!!!,ifname:%s,len:%d,max size:%d\n",
            __FUNCTION__,__LINE__,iface->ifname,
            os_strlen(iface->ifname),sizeof(wpa_s->ifname));
		return -1;
	}
	...
	if (wpa_s->ctrl_iface == NULL) {
		wpa_printf(MSG_ERROR,
			   ...
			   wpa_s->conf->ctrl_interface);
        PRINTF("%s,%d:Error!!!Failed to initialize control interface %s\n",
            __FUNCTION__,__LINE__,wpa_s->conf->ctrl_interface);
		return -1;
	}
					...
					int notify, int terminate)
{
    ...
    PRINTF("%s,%d:Deinit iface:%p\n",__FUNCTION__,__LINE__,wpa_s->ctrl_iface);
	wpa_supplicant_ctrl_iface_deinit(wpa_s, wpa_s->ctrl_iface);
	wpa_s->ctrl_iface = NULL;
				...
				unsigned int len)
{
    ...
    //PRINTF("Determining shared radio frequencies (max len %u)\n", len);
