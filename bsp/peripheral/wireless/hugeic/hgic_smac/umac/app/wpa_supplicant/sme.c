				    ...
				    int start)
{
	...
	if (bss == NULL) {
		wpa_msg(wpa_s, MSG_ERROR, "SME: No scan result available for "
			"the network");
        PRINTF("%s:No scan result available for the network,bss is NULL!\n",__FUNCTION__);
...
no_fils:
    ...
    PRINTF("SME: Trying to authenticate with " MACSTR
...
void sme_event_auth(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
    ...
    PRINTF("SME: Authentication response: peer=" MACSTR
		" auth_type=%d auth_transaction=%d status_code=%d\n",
		MAC2STR(data->auth.peer), data->auth.auth_type,
		data->auth.auth_transaction, data->auth.status_code);
	...
	if (data->auth.status_code != WLAN_STATUS_SUCCESS) {
		...
		case WLAN_AUTH_OPEN:
            ...
            PRINTF("%s,%d:SME: Trying SHARED auth\n",__FUNCTION__,__LINE__);
		...
		case WLAN_AUTH_SHARED_KEY:
            ...
            PRINTF("%s,%d:SME: Trying LEAP auth\n",__FUNCTION__,__LINE__);
			wpa_supplicant_associate(wpa_s, wpa_s->current_bss,
						 wpa_s->current_ssid);
			return;
