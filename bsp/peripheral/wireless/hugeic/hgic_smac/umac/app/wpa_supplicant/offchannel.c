...
wpas_get_tx_interface(struct wpa_supplicant *wpa_s, const u8 *src)
{
	...
	if (os_memcmp(src, wpa_s->own_addr, ETH_ALEN) == 0) {
		...
		if (wpa_s->p2p_mgmt && wpa_s != wpa_s->parent &&
		    ...
		    *wpabuf_head_u8(wpa_s->pending_action_tx) !=
            ...
            PRINTF("%s:P2P: Use GO interface %s instead of interface %s for Action TX\n",
            __FUNCTION__,
				   wpa_s->parent->ifname, wpa_s->ifname);
			return wpa_s->parent;
		}
	...
	if (iface) {
        ...
        PRINTF("%s:P2P: Use group interface %s "
			   "instead of interface %s for Action TX\n",__FUNCTION__,
			   iface->ifname, wpa_s->ifname);
		return iface;
	}
 ...
 */
static void hgic_print_hex(char *buf, int len)
{
    int i = 0;
	...
	size_t data_len, enum offchannel_send_action_result result)
{
	...
	if (data_len - wpabuf_len(wpa_s->pending_action_tx) != 24 ||
	    ...
	    os_memcmp(data + 24, wpabuf_head(wpa_s->pending_action_tx),
		      wpabuf_len(wpa_s->pending_action_tx)) != 0) 
    */
    if(os_memcmp(data + 24, wpabuf_head(wpa_s->pending_action_tx),
		      wpabuf_len(wpa_s->pending_action_tx)) != 0)

    {
		...
		wpa_hexdump_buf(MSG_MSGDUMP, "Pending TX frame",
				wpa_s->pending_action_tx);
        //PRINTF("\n%s,%d:ERROR!\n",__FUNCTION__,__LINE__);
        //PRINTF("Check data_len:%d,buf_len:%d\n",data_len,wpabuf_len(wpa_s->pending_action_tx));
        hgic_print_hex(data + 24, 32);
        hgic_print_hex(wpabuf_head(wpa_s->pending_action_tx),32);
        return;
	}
	...
	if (wpa_s->global->p2p_long_listen > 0) {
        ...
        PRINTF("%s,%d:CONTINUE long listen state!\n",__FUNCTION__,__LINE__);
			   ...
			   int no_cck)
{
    ...
    PRINTF("%s:Off-channel: Send action frame: freq=%d dst="
		   MACSTR " src=" MACSTR " bssid=" MACSTR " len=%d\n",__FUNCTION__,
		   freq, MAC2STR(dst), MAC2STR(src), MAC2STR(bssid),
		   (int) len);
	...
	if (wpa_s->pending_action_tx == NULL) {
        ...
        PRINTF("%s:Off-channel: Failed to allocate Action "
