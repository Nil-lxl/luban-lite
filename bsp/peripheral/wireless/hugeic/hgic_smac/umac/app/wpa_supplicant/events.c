...
#include "utils/common.h"//porting
#include "eapol_supp/eapol_supp_sm.h"
#include "rsn_supp/wpa.h"
#include "utils/eloop.h"//porting
#include "config.h"
#include "l2_packet/l2_packet.h"
#include "wpa_supplicant_i.h"
#include "driver_i.h"
#include "utils/pcsc_funcs.h"//porting
...
void wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
    ...
    //PRINTF("%s,%d:cur_ssid set to %p\n", __FUNCTION__, __LINE__,wpa_s->current_ssid);
    eapol_sm_notify_config(wpa_s->eapol, NULL, NULL);
    //PRINTF("%s:Clear key_mgmt!\n",__FUNCTION__);
    wpa_s->key_mgmt = 0;
                                    ...
                                    int only_first_ssid, int debug_print)
{
    ...
    if (bssid_ignore_count) {
        ...
        if (bssid_ignore_count > limit) {
            if (debug_print) {
                ...
                PRINTF("%s:skip - BSSID ignored (count=%d limit=%d)\n", __FUNCTION__,
                       bssid_ignore_count, limit);
            }
            return NULL;
        }
    ...
    if (disallowed_bssid(wpa_s, bss->bssid)) {
        if (debug_print) {
            ...
            PRINTF("%s:skip - BSSID disallowed\n", __FUNCTION__);
        }
        return NULL;
    }
    ...
    if (disabled_freq(wpa_s, bss->freq)) {
        if (debug_print) {
            ...
            PRINTF("%s:skip - channel disabled\n", __FUNCTION__);
        }
        return NULL;
    }
                           ...
                           struct wpa_ssid *ssid)
{
    ...
    PRINTF("%s:Considering connect request: reassociate: %d  selected: "
           MACSTR "  bssid: " MACSTR "  pending: " MACSTR
           "  wpa_state: %s  ssid=%p  current_ssid=%p\n", __FUNCTION__,
           wpa_s->reassociate, MAC2STR(selected->bssid),
           MAC2STR(wpa_s->bssid), MAC2STR(wpa_s->pending_bssid),
    ...
    if (wpa_s->reassociate ||
        ...
        PRINTF("%s,%d:Request association with "MACSTR"\n",
               __FUNCTION__, __LINE__, MAC2STR(selected->bssid));
...
scan_work_done:
    ...
    if (wpa_s->conf->freq_list) {
        os_free(wpa_s->conf->freq_list);
        wpa_s->conf->freq_list = NULL;
    }
    wpa_s->conf->ssid->bssid_set = 0;
    wpa_scan_results_free(scan_res);
    if (own_request && wpa_s->scan_work) {
        struct wpa_radio_work *work = wpa_s->scan_work;
        wpa_s->scan_work = NULL;
        ...
        int new_scan, int own_request)
{
    ...
    if (selected) {
        ...
        if (ssid) {
            ...
            PRINTF("%s,%d:Setup a new network\n", __FUNCTION__, __LINE__);
...
no_pfs:
    ...
    if (wpa_s->current_bss && wpa_s->current_ssid) {
        wpa_s->current_ssid->frequency = wpa_s->assoc_freq;
        os_memcpy(wpa_s->current_ssid->bssid, wpa_s->current_bss->bssid, 6);
        wpa_s->current_ssid->bssid_set = 1;
    } else {
        PRINTF("******%s.%d:Input param error,cur_bss:%p,cur_ssid:%p!!!!!!\n", 
            __FUNCTION__, __LINE__,wpa_s->current_bss,wpa_s->current_ssid);
    }
    os_free(wpa_s->conf->freq_list);
    wpa_s->conf->freq_list = NULL;
    wpa_s->conf->freq_list = os_malloc(2 * sizeof(int));
    if (wpa_s->conf->freq_list) {
        wpa_s->conf->freq_list[0] = wpa_s->assoc_freq;
        wpa_s->conf->freq_list[1] = 0;
    }
    return 0;
}
                                       ...
                                       union wpa_event_data *data)
{
    ...
    if (ft_completed) {
         ...
         * up in authenticated.ASSOCIATED
         */
        wpa_supplicant_cancel_auth_timeout(wpa_s);
        wpa_supplicant_set_state(wpa_s, WPA_COMPLETED);
...
#ifndef CONFIG_NO_SCAN_PROCESSING
        PRINTF("%s,%d:Try to reconnect to the same BSS\n", __FUNCTION__, __LINE__);
                                      ...
                                      int rssi)
{
    ...
    PRINTF("%s:Received Action frame: SA=" MACSTR
           " Category=%u DataLen=%d freq=%d MHz\n", __FUNCTION__,
           MAC2STR(mgmt->sa), category, (int) plen, freq);
                                    ...
                                    union wpa_event_data *data)
{
    ...
    if (data->assoc_reject.status_code ==
        ...
        wpa_supplicant_mark_disassoc(wpa_s);
        PRINTF("%s,%d:Try next supported DH group\n", __FUNCTION__, __LINE__);
                          ...
                          union wpa_event_data *data)
{
    ...
    switch (event) {
        ...
        case EVENT_TX_STATUS:
            ...
            PRINTF("%s:EVENT_TX_STATUS dst=" MACSTR
                   " type=%d stype=%d", __FUNCTION__,
                   MAC2STR(data->tx_status.dst),
                   data->tx_status.type, data->tx_status.stype);
#ifdef CONFIG_PASN
        ...
        case EVENT_INTERFACE_ENABLED:
            ...
            if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
                ...
                wpa_supplicant_update_channel_list(wpa_s, NULL); /*porting*/
                if (wpa_s->p2p_mgmt) {
