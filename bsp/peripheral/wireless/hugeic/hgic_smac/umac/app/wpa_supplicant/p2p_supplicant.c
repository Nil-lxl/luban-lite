				    ...
				    struct p2p_go_neg_results *res)
{
    ...
    PRINTF("P2P: Start WPS Enrollee for peer " MACSTR
		   " dev_addr " MACSTR " wps_method %d\n",
		   MAC2STR(res->peer_interface_addr),
		   MAC2STR(res->peer_device_addr), res->wps_method);
			   ...
			   int new_device)
{
	...
	if (info->p2ps_instance) {
		...
		while (len) {
            ...
            PRINTF("%s:Find dev:"MACSTR"\n",__FUNCTION__,MAC2STR(addr));
            PRINTF("%s:Find p2p_dev_addr:"MACSTR"\n",__FUNCTION__,MAC2STR(info->p2p_device_addr));
            PRINTF("%s:pri_dev_type:%s,name:'%s'\n",__FUNCTION__,
			     ...
			     const struct wpabuf *probe_resp_ie)
{
	...
	if (wpa_s->p2p_listen_work) {
        ...
        PRINTF("%s,%d:Reject start_listen since p2p_listen_work already exists\n",
	...
	if (probe_resp_ie) {
		...
		if (lwork->probe_resp_ie == NULL) {
            PRINTF("%s,%d:probe_resp_ie is NULL\n",
                __FUNCTION__,__LINE__);
			wpas_p2p_listen_work_free(lwork);
			return -1;
		}
	...
	if (radio_add_work(wpa_s, freq, "p2p-listen", 0, wpas_start_listen_cb,
			   lwork) < 0) {
        PRINTF("%s,%d:radio_add_work failed\n",
                __FUNCTION__,__LINE__);
		wpas_p2p_listen_work_free(lwork);
		return -1;
	}
			...
			if (res == ALLOWED) {
				if (reg == NULL) {
					...
					wpa_printf(MSG_DEBUG, "P2P: Add operating class %u",
						   o->op_class);
                    PRINTF("P2P: Add operating class %u\n",
						   o->op_class);
				  ...
				  const char *conf_p2p_dev)
{
	...
	if (ret < 0) {
        ...
        PRINTF("%s:Failed to create P2P interface\n",__FUNCTION__);
		return ret;
	}
	os_strlcpy(wpa_s->pending_interface_name, ifname,
		   sizeof(wpa_s->pending_interface_name));
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (wpas_p2p_mac_setup(wpa_s) < 0) {
        ...
        PRINTF("%s,%d:Error,Failed to initialize P2P random MAC address\n",__FUNCTION__,__LINE__);                
		return -1;
	}
	...
	if (wpas_p2p_setup_channels(wpa_s, &p2p.channels, &p2p.cli_channels,
        ...
        PRINTF("%s,%d:P2P: Failed to configure supported channel list!\n",__FUNCTION__,__LINE__);
		return -1;
	}
				   ...
				   unsigned int freq, unsigned int duration)
{
    ...
    PRINTF("%s:off_channel_freq=%u pending_listen_freq=%d roc_waiting_drv_freq=%d freq=%u duration=%u)\n",
		   __FUNCTION__,wpa_s->off_channel_freq, wpa_s->pending_listen_freq,
		   wpa_s->roc_waiting_drv_freq, freq, duration);
	if (wpa_s->off_channel_freq &&
		...
		wpa_printf(MSG_DEBUG, "P2P: Ignore remain-on-channel callback (off_channel_freq=%u pending_listen_freq=%d freq=%u duration=%u)",
			   wpa_s->off_channel_freq, wpa_s->pending_listen_freq,
			   freq, duration);
        PRINTF("P2P: Ignore remain-on-channel callback (off_channel_freq=%u pending_listen_freq=%d freq=%u duration=%u)\n",
			   wpa_s->off_channel_freq, wpa_s->pending_listen_freq,
			   freq, duration);
	}
}
