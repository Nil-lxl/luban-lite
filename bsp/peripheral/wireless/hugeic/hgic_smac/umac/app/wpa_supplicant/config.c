...
set:
	...
	if ((*dst == NULL && tmp == NULL) ||
     ...
     /*    
        PRINTF("%s,%d:Error!tmp:%p,dst:%p,prev_len:%d,res_len:%d\n",
        __FUNCTION__,__LINE__,tmp,*dst,prev_len,res_len);
        if(*dst != NULL) {
            PRINTF("%s:dest:%s\n",__FUNCTION__,*dst);
        }
        if(tmp != NULL) {
	...
	for (i = 0; i < NUM_SSID_FIELDS; i++) {
		...
		if (ret < 0) {
			if (line) {
                ...
                PRINTF("%s:Line %d: failed to "
					   "parse %s '%s'.\n", __FUNCTION__,line, var, value);
			}
            PRINTF("%s,%d:Error!parse:%p,ssid:%s,var:%s,value:%s\n",
            __FUNCTION__,__LINE__,field->parser,ssid->ssid,var,value);
			ret = -1;
		}
#ifdef CONFIG_SAE
	...
	if (i == NUM_SSID_FIELDS) {
		if (line) {
			wpa_printf(MSG_ERROR, "Line %d: unknown network field "
				   "'%s'.", line, var);
            PRINTF("%s:Line %d: unknown network field "
				   "'%s'.\n", __FUNCTION__,line, var);
		}
        PRINTF("%s,%d:Error!\n",__FUNCTION__,__LINE__);
		ret = -1;
	}
