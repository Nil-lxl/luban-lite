	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "EAPOL::heldPeriod") == 0) {
		...
		if (enabled && !wpa_s->global->p2p) {
            ret = -1;
            PRINTF("%s,%d:WIFI display set failed,%d,p2p:%p\n",__FUNCTION__,__LINE__,
                enabled,wpa_s->global->p2p);
        }
		else
					    ...
					    char *buf, size_t buflen)
{
    ...
    PRINTF("%s,%d:Get status,wpa_state:%d\n",__FUNCTION__,__LINE__,wpa_s->wpa_state);
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	id = atoi(cmd);
	wpa_printf(MSG_DEBUG, "CTRL_IFACE: SET_NETWORK id=%d cmd='%s'",
		   id, name);
    PRINTF("%s:set network_id:%d,name:%s\n",__FUNCTION__,id,name);
	wpa_hexdump_ascii_key(MSG_DEBUG, "CTRL_IFACE: value",
	...
	if (ssid == NULL) {
		wpa_printf(MSG_DEBUG, "CTRL_IFACE: Could not find network "
			   "id=%d", id);
        PRINTF("%s:Could not find network id:%d\n",__FUNCTION__,id);
		return -1;
	}
					 ...
					 size_t buflen)
{
	...
	if (os_strncmp(cmd, "RANGE=", 6) == 0) {
		...
		if (hwaddr_aton(cmd + 13, bssid) == 0)
			...
			bss = NULL;
        PRINTF("%s,%d:cur bss set to %p\n",__FUNCTION__,__LINE__,bss);
#endif /* CONFIG_P2P */
...
static int wpas_ctrl_iface_driver_event(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	} else if (os_strcmp(cmd, "CHANNEL_LIST_CHANGED") == 0) { /*porting: update channel list*/
		ev = EVENT_CHANNEL_LIST_CHANGED;
	} else {
