...
/*porting*/
struct wpas_scan_match scan_match;
void umac_set_scan_ssid(char *ssid)
{
    if (ssid) {
        strncpy(scan_match.scan_ssid, ssid, 31);
    } else {
        memset(scan_match.scan_ssid, 0, 32);
    }
}
void umac_set_scan_bssid(char *bssid)
{
    if (bssid) {
        memcpy(scan_match.scan_bssid, bssid, 6);
    } else {
        memset(scan_match.scan_bssid, 0, 6);
    }
}
int umac_scan_result_check(const u8 *ssid, int ssid_len, const u8 *bssid)
{
    return (memcmp(scan_match.scan_bssid, bssid, 6) == 0 && 
            memcmp(scan_match.scan_ssid, ssid, ssid_len) == 0);
}
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->scanning) {
		...
		wpa_supplicant_req_scan(wpa_s, 1, 0);//porting
		return;
	}
	...
	if (p2p_in_prog && p2p_in_prog != 2 &&
		...
		wpa_supplicant_req_scan(wpa_s, 5, 0);//porting
		return;
	}
    ...
    umac_set_scan_ssid(0); /*porting*/
    umac_set_scan_bssid(0);
	...
	if ((wpa_s->p2p_in_provisioning || wpa_s->show_group_started) &&
        ...
        PRINTF("P2P: Use specific SSID for scan during P2P group formation (p2p_in_provisioning=%d show_group_started=%d)\n",
			   wpa_s->p2p_in_provisioning,
			   wpa_s->show_group_started);
	...
	if (wpa_s->p2p_in_invitation) {
		if (wpa_s->current_ssid) {
            ...
            PRINTF("P2P: Use specific SSID for scan during invitation!\n");
            ...
            PRINTF("P2P: No specific SSID known for scan during invitation\n");
		}
		goto ssid_list_set;
	}
#endif /* CONFIG_P2P */
	...
	if (wpa_s->last_scan_req != MANUAL_SCAN_REQ &&
		...
		params.num_ssids = 1;
        umac_set_scan_ssid((char *)ssid->ssid); /*porting*/
        if(ssid->bssid_set){
            umac_set_scan_bssid((char *)ssid->bssid);
        }
		...
		while (ssid) {
			if (!wpas_network_disabled(wpa_s, ssid) &&
				...
				params.num_ssids++;
                /*porting*/
                umac_set_scan_ssid((char *)ssid->ssid);
                if(ssid->bssid_set){
                    umac_set_scan_bssid((char *)ssid->bssid);
                }else{
                    umac_set_scan_bssid(0);
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	if (wpa_s->p2p_mgmt) {
		wpa_dbg(wpa_s, MSG_DEBUG,
			...
			sec, usec);
        PRINTF("%s:Ignore scan request (%d.%06d sec) on p2p_mgmt interface",__FUNCTION__,
			sec, usec);
		return;
	}
