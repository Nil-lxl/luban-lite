...
//porting
int hostapd_config_edca_auto_params(struct hostapd_wmm_ac_params *ac,
			  const char *name, const char *val)
{
	const char *pos = val;
    int i = 0;
    ...
    do{
        ...
        if(ptr) *ptr = 0;
        PRINTF("parse:%s\r\n", pos);
        if(4 != sscanf(pos, "%d,%d,%d,%d", &ac[i].txop_limit, &ac[i].cwmin, &ac[i].cwmax, &ac[i].aifs)){
            PRINTF("invalid edca_auto_params:%s\r\n", pos);
            return -1;
        }
        if(ptr) {
            *ptr = ';';
