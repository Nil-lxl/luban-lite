...
struct wpa_driver_ops {
	 ...
	 */
	void * (*global_init)(void *ctx, char *ifname); /*porting*/
...
void hapd_supplicant_event(void *ctx, enum wpa_event_type event,
			  union wpa_event_data *data); //porting
...
void wpa_supplicant_event_global(void *ctx, enum wpa_event_type event,
				 union wpa_event_data *data);
//porting
void hapd_supplicant_event_global(void *ctx, enum wpa_event_type event,
				 union wpa_event_data *data);
...
static inline void drv_event_assoc(void *ctx, const u8 *addr, const u8 *ie,
				   size_t ielen, int reassoc, u8 hostapd)
{
    ...
    if(hostapd) //porting
        hapd_supplicant_event(ctx, EVENT_ASSOC, &event);
    else
        wpa_supplicant_event(ctx, EVENT_ASSOC, &event);
}

static inline void drv_event_disassoc(void *ctx, const u8 *addr, u8 hostapd)
{
	union wpa_event_data event;
	os_memset(&event, 0, sizeof(event));
	event.disassoc_info.addr = addr;
    if(hostapd) //porting
    	hapd_supplicant_event(ctx, EVENT_DISASSOC, &event);
    else
        wpa_supplicant_event(ctx, EVENT_DISASSOC, &event);
}

static inline void drv_event_eapol_rx(void *ctx, const u8 *src, const u8 *data,
				      size_t data_len, u8 hostapd)
{
	union wpa_event_data event;
	os_memset(&event, 0, sizeof(event));
    ...
    if(hostapd)//porting
	    hapd_supplicant_event(ctx, EVENT_EAPOL_RX, &event);
	else
