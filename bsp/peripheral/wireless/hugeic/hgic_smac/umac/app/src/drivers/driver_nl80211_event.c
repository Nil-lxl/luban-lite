...
//porting
char * nl80211_command_to_string(enum nl80211_commands cmd)
{
			    ...
			    const u8 *frame, size_t len)
{
	...
	drv->event(drv->ctx, EVENT_AUTH, &event);//porting
}
				     ...
				     struct wmm_params *wmm_params)
{
	...
	//static struct nla_policy wme_policy[NL80211_STA_WME_MAX + 1] = {     //porting
			     ...
			     struct nlattr *req_ie)
{
	...
	if (status != WLAN_STATUS_SUCCESS) {
		...
		drv->event(drv->ctx, EVENT_ASSOC_REJECT, &event);//porting
		return;
	}
	...
	drv->event(drv->ctx, EVENT_ASSOC, &event);//porting
}
				  ...
				  struct nlattr *by_ap)
{
	...
	drv->event(drv->ctx, EVENT_DEAUTH, &data);//porting
}
				 ...
				 int finished)
{
	...
	drv->event(bss->ctx, finished ?//porting
			       ...
			       enum nl80211_commands cmd, struct nlattr *addr)
{
	...
	drv->event(drv->ctx, ev, &event);//porting
}
			    ...
			    const u8 *frame, size_t len)
{
	...
	wpa_printf(MSG_DEBUG,
		   ...
		   (unsigned int) len);
/*
    PRINTF("%s:RX frame da=" MACSTR " sa=" MACSTR " bssid=" MACSTR
		   " freq=%d ssi_signal=%d fc=0x%x seq_ctrl=0x%x stype=%u (%s) len=%u\n",__FUNCTION__
		   MAC2STR(mgmt->da), MAC2STR(mgmt->sa), MAC2STR(mgmt->bssid),
		   rx_freq, ssi_signal, fc,
		   le_to_host16(mgmt->seq_ctrl), stype, fc2str(fc),
...
#if 0 /*porting: support send EAPOL frame*/
...
/*
        PRINTF("nl80211: %s:Frame TX status: cookie=0x%llx%s (ack=%d)\n",__FUNCTION__,
			   (long long unsigned int) cookie_val,
			   cookie_val == drv->send_frame_cookie ?
			   " (match)" : " (unknown)", ack != NULL);
*/
	...
	drv->event(drv->ctx, EVENT_TX_STATUS, &event);//porting
}
				       ...
				       const u8 *frame, size_t len)
{
	...
	drv->event(drv->ctx, type, &event);//porting
}
				     ...
				     const u8 *frame, size_t len)
{
	...
	drv->event(drv->ctx, EVENT_UNPROT_BEACON, &event);//porting
}
					   ...
					   struct nlattr *tb[])
{
	...
	bss->drv->event(bss->ctx, EVENT_MICHAEL_MIC_FAILURE, &data);//porting
}
...
/*
    PRINTF("nl80211: Remain-on-channel event (cancel=%d "
		   "freq=%u channel_type=%u duration=%u cookie=0x%llx (%s))\n",
		   cancel_event, freq, chan_type, duration,
		   (long long unsigned int) cookie,
		   cookie == drv->remain_on_chan_cookie ? "match" : "unknown");
*/
	...
	drv->event(drv->ctx, cancel_event ?//porting
				...
				struct nlattr *tb[])
{
	...
	drv->event(drv->ctx, EVENT_FT_RESPONSE, &data);//porting
}
				...
				struct nlattr *tb[])
{
	...
	drv->event(bss->ctx, EVENT_UPDATE_DH, &data);//porting
}
...
#define MAX_REPORT_FREQS 50
	...
	drv->event(drv->ctx, EVENT_SCAN_RESULTS, &event);//porting
}
			      ...
			      struct nlattr *tb[])
{
	...
	if (cqm[NL80211_ATTR_CQM_PKT_LOSS_EVENT]) {
		...
		drv->event(drv->ctx, EVENT_STATION_LOW_ACK, &ed);//porting
		return;
	}

	if (cqm[NL80211_ATTR_CQM_BEACON_LOSS_EVENT]) {
		wpa_printf(MSG_DEBUG, "nl80211: Beacon loss event");
		drv->event(drv->ctx, EVENT_BEACON_LOSS, NULL);//porting
		return;
	}
	...
	drv->event(drv->ctx, EVENT_SIGNAL_CHANGE, &ed);//porting
}
				       ...
				       struct nlattr **tb)
{
	...
	drv->event(drv->ctx, EVENT_NEW_PEER_CANDIDATE, &data);//porting
}
				      ...
				      struct nlattr **tb)
{
    ...
    PRINTF("nl80211: New station " MACSTR"\n", MAC2STR(addr));
	...
	if (is_ap_interface(drv->nlmode) && drv->device_ap_sme) {
		...
		drv_event_assoc(bss->ctx, addr, ies, ies_len, 0, drv->hostapd);
		return;
	}
	...
	drv->event(bss->ctx, EVENT_IBSS_RSN_START, &data);//porting
}
				      ...
				      struct nlattr **tb)
{
	...
	if (is_ap_interface(drv->nlmode) && drv->device_ap_sme) {
		drv_event_disassoc(bss->ctx, addr, drv->hostapd);
		return;
	}
	...
	drv->event(bss->ctx, EVENT_IBSS_PEER_LOST, &data);//porting
}
					...
					struct nlattr **tb)
{
	...
	if (!tb[NL80211_ATTR_MAC] ||
	    ...
	    nla_parse_nested(rekey_info, MAX_NL80211_REKEY_DATA,
			     tb[NL80211_ATTR_REKEY_DATA], NULL, NULL) ||//porting
	...
	drv->event(drv->ctx, EVENT_DRIVER_GTK_REKEY, &data);//porting
}
					  ...
					  struct nlattr **tb)
{
	...
	if (!tb[NL80211_ATTR_PMKSA_CANDIDATE] ||
	    nla_parse_nested(cand, MAX_NL80211_PMKSA_CANDIDATE,
			     tb[NL80211_ATTR_PMKSA_CANDIDATE], NULL, NULL) ||//porting
	...
	drv->event(drv->ctx, EVENT_PMKID_CANDIDATE, &data);//porting
}
				       ...
				       struct nlattr **tb)
{
	...
	drv->event(drv->ctx, EVENT_DRIVER_CLIENT_POLL_OK, &data);//porting
}
				    ...
				    struct nlattr **tb)
{
	...
	drv->event(drv->ctx, EVENT_TDLS, &data);//porting
}


static void nl80211_driver_stop_ap(struct wpa_driver_nl80211_data *drv, /*porting*/
			    struct nlattr **tb)
{
	drv->event(drv->ctx, EVENT_INTERFACE_UNAVAILABLE, NULL);//porting
}
					 ...
					 struct nlattr **tb)
{
	...
	drv->event(drv->ctx, EVENT_CONNECT_FAILED_REASON, &data);//porting
}
				...
				struct nlattr **tb)
{
	...
	case NL80211_RADAR_DETECTED:
		drv->event(drv->ctx, EVENT_DFS_RADAR_DETECTED, &data);//porting
		break;
	case NL80211_RADAR_CAC_FINISHED:
		drv->event(drv->ctx, EVENT_DFS_CAC_FINISHED, &data);//porting
		break;
	case NL80211_RADAR_CAC_ABORTED:
		drv->event(drv->ctx, EVENT_DFS_CAC_ABORTED, &data);//porting
		break;
	case NL80211_RADAR_NOP_FINISHED:
		drv->event(drv->ctx, EVENT_DFS_NOP_FINISHED, &data);//porting
		break;
	case NL80211_RADAR_PRE_CAC_EXPIRED:
		drv->event(drv->ctx, EVENT_DFS_PRE_CAC_EXPIRED,//porting
				     &data);
		break;
	case NL80211_RADAR_CAC_STARTED:
		drv->event(drv->ctx, EVENT_DFS_CAC_STARTED, &data);//porting
		break;
	default:
				   ...
				   int wds)
{
	...
	drv->event(drv->ctx, EVENT_RX_FROM_UNKNOWN, &event);//porting
}
				   ...
				   const u8 *data, size_t len)
{
	...
	drv->event(drv->ctx, EVENT_AVOID_FREQUENCIES, &event);//porting
				   ...
				   const u8 *data, size_t len)
{
	...
	drv->event(drv->ctx, EVENT_ACS_CHANNEL_SELECTED, &event);//porting
}
					   ...
					   u8 *data, size_t len)
{
	...
	drv->event(drv->ctx, EVENT_SCAN_STARTED, &event);//porting
}
					  ...
					  u8 *data, size_t len)
{
	...
	wpa_printf(MSG_DEBUG,
		   ...
		   event.p2p_lo_stop.reason_code);
	drv->event(drv->ctx, EVENT_P2P_LO_STOP, &event);//porting
}
				     ...
				     struct nlattr *tb[])
{
	...
	drv->event(drv->ctx, EVENT_CHANNEL_LIST_CHANGED, &data);//porting
}
...
static void nl80211_dump_freq(const char *title, struct nlattr *nl_freq)
{
	...
	nla_parse(tb, NL80211_FREQUENCY_ATTR_MAX,
		  nla_data(nl_freq), nla_len(nl_freq), NULL, NULL);//porting
				  ...
				  struct nlattr **tb)
{
	...
	wpa_printf(MSG_DEBUG,
		   ...
		   event.external_auth.key_mgmt_suite);
	drv->event(drv->ctx, EVENT_EXTERNAL_AUTH, &event);//porting
}
				    ...
				    struct nlattr **tb)
{
	...
	drv->event(drv->ctx, EVENT_PORT_AUTHORIZED, NULL);//porting
}
					    ...
					    struct nlattr **tb)
{
	...
	drv->event(drv->ctx, EVENT_STATION_OPMODE_CHANGED, &ed);//porting
}
				       ...
				       struct nlattr **tb)
{
	...
	case ETH_P_PAE:
		drv_event_eapol_rx(drv->ctx, src_addr,
				   ...
				   nla_len(tb[NL80211_ATTR_FRAME]), drv->hostapd);
		break;
	default:
				     ...
				     struct nlattr *ack, struct nlattr *cookie)
{
	...
	drv->event(drv->ctx, EVENT_EAPOL_TX_STATUS, &event);//porting
}
				 ...
				 struct nlattr **tb)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Drv Event %d (%s) received for %s",
		   cmd, nl80211_command_to_string(cmd), bss->ifname);
/*
    PRINTF("nl80211: %s:Drv Event %d (%s) received for %s\n",__FUNCTION__,
		   cmd, nl80211_command_to_string(cmd), bss->ifname);

    if(cmd == NL80211_CMD_FRAME_WAIT_CANCEL) {
        PRINTF("%s:Recv WAIT_CANCEL!\n",__FUNCTION__);
    }
    */
	...
	case NL80211_CMD_TRIGGER_SCAN:
		...
		drv->event(drv->ctx, EVENT_SCAN_STARTED, NULL);//porting
		break;
	...
	case NL80211_CMD_SCHED_SCAN_STOPPED:
		...
		drv->event(drv->ctx, EVENT_SCHED_SCAN_STOPPED, NULL);//porting
		break;
	...
	case NL80211_CMD_STOP_AP:
		nl80211_driver_stop_ap(drv, tb); /*porting*/
		break;
	case NL80211_CMD_VENDOR:
		nl80211_vendor_event(drv, tb);
		break;
#ifdef CONFIG_MAC80211_MESH //porting
	...
	default:
		wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Ignored unknown event "
			"(cmd=%d)", cmd);
        //PRINTF("%s:Ignore unknow event:%d\n",__FUNCTION__,cmd);
		break;
	}
}
...
int process_bss_event(struct nl_msg *msg, void *arg)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: BSS Event %d (%s) received for %s",
		   ...
		   bss->ifname);
/*
    PRINTF("%s:nl80211: BSS Event %d (%s) received for %s\n",__FUNCTION__,
		   gnlh->cmd, nl80211_command_to_string(gnlh->cmd),
		   bss->ifname);
*/
	switch (gnlh->cmd) {
	...
	case NL80211_CMD_EXTERNAL_AUTH:
		...
		break;
#if 0 /*porting: not support control port*/
