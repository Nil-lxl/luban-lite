...
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc)
{
	...
	if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, freq, 1024 * tu / 1000,
        ...
        PRINTF("%s,%d:Failed to start listen mode\n",__FUNCTION__,__LINE__);
		   ...
		   size_t ies_len, int scan_res)
{
	...
	if (dev->info.config_methods == 0 &&
        ...
        PRINTF("Do not report peer " MACSTR
			" with unknown config methods!\n", MAC2STR(addr));
		return 0;
	}
...
{
	if (status != 0) {
		p2p_dbg(p2p, "Scan request failed,ret:%d\n",status);
		  ...
		  unsigned int pref_freq, u16 oob_pw_id)
{
    ...
    PRINTF("%s,%d:Set wps_method:%d\n",__FUNCTION__,__LINE__,wps_method);
	dev->wps_method = wps_method;
	dev->oob_pw_id = oob_pw_id;
	dev->status = P2P_SC_SUCCESS;
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
    ...
    PRINTF("%s,%d:set wps_method:%d\n",__FUNCTION__,__LINE__,res.wps_method);
...
//static unsigned char reply_buff[4096];
			      ...
			      const u8 *data, size_t len, int rx_freq)
{
	...
	case P2P_GO_NEG_REQ:
        ...
        //wpas_cli("w0","P2P_CONNECT 92:3d:70:75:c4:43 pbc go_intent=1",&reply_buff, sizeof(reply_buff));
		break;
					 ...
					 u8 query_count)
{
	...
	if (p2p->go_neg_peer) {
        ...
        PRINTF("%s,%d:Check wps_method:%d\n",__FUNCTION__,__LINE__,p2p->go_neg_peer->wps_method);
...
int p2p_unauthorize(struct p2p_data *p2p, const u8 *addr)
{
	...
	dev->flags &= ~P2P_DEV_WAIT_GO_NEG_RESPONSE;
	dev->flags &= ~P2P_DEV_WAIT_GO_NEG_CONFIRM;
    PRINTF("%s,%d:Disable P2P_DEV_WAIT_GO_NEG_RESPONSE!\n",__FUNCTION__,__LINE__);    
    PRINTF("%s,%d:Set wps_method:%d\n",__FUNCTION__,__LINE__,dev->wps_method);
			...
			enum p2p_send_action_result result)
{
    ...
    if(p2p->pending_action_state == P2P_PENDING_GO_NEG_CONFIRM) {
        PRINTF("%s:PENDIN CONFIRM!\n",__FUNCTION__);
    }
