...
static inline void * os_calloc(size_t nmemb, size_t size)
{
	if (size && nmemb > (~(size_t) 0) / size)
		return NULL;
	return ZALLOC(nmemb * size); /*porting*/
}
...
#ifndef os_free
static inline void os_free(unsigned char *p)
{
    if(!p) {
        return;
    }
...
//extern void *osal_zalloc(unsigned long size);
