...
void ap_free_sta(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	os_free(sta);
    //PRINTF("del sta, update beacon\r\n");
    hapd->parameter_set_count++; //porting
...
struct sta_info * ap_sta_add(struct hostapd_data *hapd, const u8 *addr)
{
    ...
    //PRINTF("add new sta, update beacon\r\n");
    hapd->parameter_set_count++; //porting
    ieee802_11_update_beacons(hapd->iface);
	return sta;
}
...
void ap_sta_state_notify(struct hostapd_data *hapd,struct sta_info *sta,
                               int conn_state)
{
    int  len = 1;
    char state[8];
    struct net_device *dev = net_device_get_by_name(hapd->ifname);

    if(dev == NULL) {
        PRINTF("%s,%d:Can not find netdev:%s\n",__FUNCTION__,__LINE__,hapd->ifname);
        return;
    }
    ...
    switch (conn_state) {
        case 1:
            state[0] = 9;      
            memcpy(state + 1, sta->addr, ETH_ALEN);
            len += ETH_ALEN;
            PRINTF("AP:sta connect success,mac:"MACSTR"\r\n",MAC2STR(sta->addr));
            break;
       ...
       case 0:
            ...
            memcpy(state + 1, sta->addr, ETH_ALEN);
            len += ETH_ALEN;
            PRINTF("AP:sta disconnected success,mac:"MACSTR"\r\n",MAC2STR(sta->addr));
            break;
    ...
    net_device_event(dev, 1, state, len);//HGIC_EVENT_STATE_CHG
}
			   ...
			   int authorized)
{
	...
	if (authorized) {
        ...
        ap_sta_state_notify(hapd, sta, 1);
	} else {
        ...
        ap_sta_state_notify(hapd, sta, 0);
	}
