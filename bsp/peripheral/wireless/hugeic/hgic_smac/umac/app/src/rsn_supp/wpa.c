				  ...
				  const u8 *pmkid)
{
	...
	if (pmkid && sm->cur_pmksa &&
		...
		if (res == 0) {
			...
			sm->pmk_len = pmk_len;
            PRINTF("%s,%d:PMK:Check pmk_len:%d\n",__FUNCTION__,__LINE__,sm->pmk_len);
					  ...
					  size_t key_data_len, u16 ver)
{
	...
	wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: RX message 1 of Group Key "
		"Handshake from " MACSTR " (ver=%d),proto:%d", MAC2STR(src_addr), ver,sm->proto);
	...
	if (ret) {
        PRINTF("%s,%d:Error,ret:%d,proto:%d\n",__FUNCTION__,__LINE__,ret,sm->proto);
        goto failed;        
    }
		    ...
		    const u8 *pmkid, const u8 *bssid)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "WPA: Set PMK based on external data",
			pmk, pmk_len);
    //PRINTF("%s,%d:set pmk_len:%d\n",__FUNCTION__,__LINE__,pmk_len);
	sm->pmk_len = pmk_len;
...
void wpa_sm_set_pmk_from_pmksa(struct wpa_sm *sm)
{
	...
	if (sm->cur_pmksa) {
        ...
        PRINTF("%s,%d:set pmk_len:%d\n",__FUNCTION__,__LINE__,sm->cur_pmksa->pmk_len);
		sm->pmk_len = sm->cur_pmksa->pmk_len;
		os_memcpy(sm->pmk, sm->cur_pmksa->pmk, sm->pmk_len);
	} else {
		wpa_printf(MSG_DEBUG, "WPA: No current PMKSA - clear PMK");
        PRINTF("%s,%d:set pmk_len to 0\n",__FUNCTION__,__LINE__);
		sm->pmk_len = 0;
		      ...
		      size_t len)
{
	...
	if (!sm->cur_pmksa && elems.wrapped_data) {
        ...
        PRINTF("%s:%d:Check pmk_len:%d\n",__FUNCTION__,__LINE__,sm->pmk_len);XXX
			   ...
			   const u8 *resp_ies, size_t resp_ies_len)
{
	...
	if (group == 19) {
		...
		res = -1;
		hash_len = 0;
        PRINTF("%s,%d:PMK_LEN:Error!group:%d\n",__FUNCTION__,__LINE__,group);
	}
	pub = wpabuf_zeropad(pub, prime_len);
	if (res < 0 || !pub) {
		wpabuf_free(pub);
	...
	if (res < 0) {
        PRINTF("%s,%d:PMK:Error!res:%d,group:%d\n",__FUNCTION__,__LINE__,res,group);
		sm->pmk_len = 0;
		return -1;
	}
    PRINTF("%s,%d:PMK_LEN:%d\n",__FUNCTION__,__LINE__,hash_len);
	sm->pmk_len = hash_len;
