			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		p2p_dbg(p2p, "Not ready for GO negotiation with " MACSTR,
			MAC2STR(sa));
        PRINTF("%s:ERROR:Check p2pdev:%pmethod:%d,pw_id:%d,pwid:%d\n",
            __FUNCTION__,dev,dev->wps_method,p2p->authorized_oob_dev_pw_id,msg.dev_password_id);
