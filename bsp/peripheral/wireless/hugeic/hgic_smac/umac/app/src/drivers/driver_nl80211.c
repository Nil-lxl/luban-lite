...
#else
#define nl_socket_get_fd(sock) (sock)
#define nl_socket_set_nonblocking(...)
static struct nl_handle *nl_create_handle(char *ifname)
{
    return nl80211_handle_alloc(ifname);
}
#endif
...
{
#if 1 /*porting*/
    eloop_register_read_sock((int)nl_socket_get_fd(*handle), handler,
                             eloop_data, *handle);
#else
    /*
...
{
#if 1 /*porting*/
    eloop_unregister_read_sock((int)nl_socket_get_fd(*handle));
#else
    if (!persist)
        *handle = (void *)(((intptr_t) * handle) ^
                           ELOOP_SOCKET_INVALID);
...
static int ack_handler_cookie(struct nl_msg *msg, void *arg)
{
    ...
    *ext_arg->err = 0;
    ...
    nla_parse(tb, NLMSGERR_ATTR_MAX, attrs, attr_len, NULL, NULL); //porting
                         ...
                         void *arg)
{
    ...
    *ret = err->error;
    ...
    nla_parse(tb, NLMSGERR_ATTR_MAX, attrs, len, NULL, NULL); //porting
...
//#else
    os_memset(msg->head, 0, msg->end - msg->head);
#endif
}


static int send_and_recv(struct nl80211_global *global,
                         struct nl_sock *handle, struct nl_msg *msg, //porting
                         ...
                         void *ack_data)
{
    ...
    if (!msg || global == NULL)
    {        
        PRINTF("%s,%d:Input param error:msg:%p,global:%p\n",__FUNCTION__,__LINE__,msg,global);
        return -ENOMEM;
    }
    ...
    if (handle->err) {
        PRINTF("%s,%d: netlink process error:%d\r\n", __FUNCTION__, __LINE__, handle->err);
    }

    if (down_timeout(&handle->done, 1000)) {
        PRINTF("%s,%d: timeout\r\n", __FUNCTION__, __LINE__);
        err = -ETIMEDOUT;
    } else {
        if (!handle->err) {
            ...
            if (err) {
                PRINTF("%s,%d:nl recvmsgs error:%d\r\n", __FUNCTION__, __LINE__, err);
            }
        } else {
            err = handle->err;
        }
                                    ...
                                    void *ack_data)
{
    ...
    if((int)handle == -1) {
        PRINTF("%s:Invaild input handle:%d\n",__FUNCTION__,__LINE__);
    }
...
struct nl_sock *get_connect_handle(struct i802_bss *bss)
{
    ...
    if ((bss->drv->capa.flags2 & WPA_DRIVER_FLAGS2_CONTROL_PORT_RX) ||
        bss->use_nl_connect) {
            if(bss->nl_connect == NULL) {
                PRINTF("%s,%d:bss->nl_connect is NULL!\n",__FUNCTION__,__LINE__);
            }
            if(bss->drv->capa.flags2 & WPA_DRIVER_FLAGS2_CONTROL_PORT_RX) {
                PRINTF("%s,%d:Support WPA_DRIVER_FLAGS2_CONTROL_PORT_RX!\n",__FUNCTION__,__LINE__);
            }
            if(bss->use_nl_connect) {
                PRINTF("%s,%d:Support use_nl_connect!\n",__FUNCTION__,__LINE__);
            }
            return bss->nl_connect;
        }
    PRINTF("%s,%d:nlsocket is NULL!nl_connect:%x\n",__FUNCTION__,__LINE__,bss->use_nl_connect);
    return NULL;
#else 
    if(bss)
        return bss->nl_connect;
                  ...
                  struct nl_msg *msg, int flags, uint8_t cmd)
{
    if (TEST_FAIL())
    { return NULL; }
    return genlmsg_put(msg, 0, 0, NULL, flags, cmd); /*porting*/
}
...
{
    struct nlattr **tb = NULL; //porting
    struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
    struct wiphy_idx_data *info = arg;

    tb = MALLOC(sizeof(int) * (NL80211_ATTR_MAX + 1)); /*porting*/
    if (tb == NULL)
    { return NL_SKIP; }

    nla_parse(tb, NL80211_ATTR_MAX, genlmsg_attrdata(gnlh, 0),
              genlmsg_attrlen(gnlh, 0), NULL, NULL); /*porting*/
...
static int process_beacon_event(struct nl_msg *msg, void *arg)
{
    ...
    dl_list_for_each(drv, &w->drvs, struct wpa_driver_nl80211_data,
        ...
        drv->event(drv->ctx, EVENT_RX_MGMT, &event);  /*porting*/
    }
    FREE(tb); /*porting*/
    return NL_SKIP;
}
...
nl80211_get_wiphy_data_ap(struct i802_bss *bss)
{
    ...
    if (ieee80211_freq_to_chan(bss->freq, &channel) !=
        ...
        w->nl_beacons = nl_create_handle(bss->ifname); /*porting*/
    ...
    int ifindex, const char *ifname)
{
    ...
    if (drv && os_strcmp(drv->first_bss->ifname, ifname) == 0) {
        if (if_nametoindex(drv->first_bss->ifname) == 0) {
            ...
            PRINTF("nl80211: Interface %s does not exist - ignore RTM_NEWLINK\n",
                   drv->first_bss->ifname);
            return;
        }
        if (!drv->if_removed)
        ...
        wpa_printf(MSG_DEBUG, "nl80211: Mark if_removed=0 for %s based on RTM_NEWLINK event",
                   drv->first_bss->ifname);
        PRINTF("nl80211: Mark if_removed=0 for %s based on RTM_NEWLINK event\n",
               drv->first_bss->ifname);
        drv->if_removed = 0;
    }
    ...
    if (drv)
    { drv->event(drv->ctx, EVENT_INTERFACE_STATUS, &event); } //porting
    else
        drv->global_event(global->ctx, EVENT_INTERFACE_STATUS, //porting
                          &event);
}
        ...
        u8 *buf, size_t len)
{
    ...
    PRINTF("RTM_NEWLINK: ifi_index=%d ifname=%s%s ifi_family=%d ifi_flags=0x%x (%s%s%s%s)\n",
           ifi->ifi_index, ifname, extra, ifi->ifi_family,
           ifi->ifi_flags,
           (ifi->ifi_flags & IFF_UP) ? "[UP]" : "",
           (ifi->ifi_flags & IFF_RUNNING) ? "[RUNNING]" : "",
...
{
    struct nlattr **tb = NULL; //porting
    struct genlmsghdr *gnlh = (struct genlmsghdr *)nlmsg_data(nlmsg_hdr(msg)); //porting
    ...
    if (!tb[NL80211_ATTR_STA_INFO] ||
        nla_parse_nested(sinfo, NL80211_STA_INFO_MAX,
                         tb[NL80211_ATTR_STA_INFO], NULL, NULL)) { //porting
        FREE(tb);
        return NL_SKIP;
    }
    if (!sinfo[NL80211_STA_INFO_SIGNAL]) { //porting
        FREE(tb);
        return NL_SKIP;
    }
...
static int nl80211_get_country(struct nl_msg *msg, void *arg)
{
    ...
    tb_msg = MALLOC(sizeof(int) * (NL80211_ATTR_MAX + 1)); /*porting*/
    if (tb_msg == NULL)
    { return NL_SKIP; }
...
static int wpa_driver_nl80211_init_nl_global(struct nl80211_global *global)
{
    ...
    global->nl = nl_create_handle(global->ifname); /*porting*/
    if (global->nl == NULL)
    { goto err; }
#if 0//porting
...
#endif
    global->nl_event = nl_create_handle(global->ifname); /*porting*/
    if (global->nl_event == NULL)
    { goto err; }

    ret = nl_get_multicast_id("scan");//porting
    if (ret >= 0)
    ...
    ret = nl_get_multicast_id("mlme");//porting
    if (ret >= 0)
    { ret = nl_socket_add_membership(global->nl_event, ret); }
    if (ret < 0) {
        wpa_printf(MSG_ERROR, "nl80211: Could not add multicast "
...
#if 0//porting
    ret = nl_get_multicast_id("regulatory");//porting
    if (ret >= 0)
    { ret = nl_socket_add_membership(global->nl_event, ret); }
    if (ret < 0) {
    ...
    ret = nl_get_multicast_id("vendor");//porting
    if (ret >= 0)
    { ret = nl_socket_add_membership(global->nl_event, ret); }
    if (ret < 0) {
        wpa_printf(MSG_DEBUG, "nl80211: Could not add multicast "
...
static int nl80211_init_connect_handle(struct i802_bss *bss)
{
    ...
    bss->nl_connect = nl_create_handle(bss->ifname);//porting
    if (!bss->nl_connect)
    { return -1; }
        ...
        const char *driver_params)
{
    ...
    drv->hostapd = !!hostapd;
    drv->eapol_sock = -1;
    if (drv->hostapd) { /*porting*/
#ifdef HOSTAPD
        drv->event = hapd_supplicant_event;
        drv->global_event = hapd_supplicant_event_global;
#endif
    } else {
#ifdef CONFIG_WPAS
        drv->event = wpa_supplicant_event;
        drv->global_event = wpa_supplicant_event_global;
#endif
    }
...
skip_wifi_status:
    ...
    if (drv->global) {
        //nl80211_check_global(drv->global); //porting
...
static int nl80211_alloc_mgmt_handle(struct i802_bss *bss)
{
    ...
    bss->nl_mgmt = nl_create_handle(bss->ifname); /*porting*/
    if (bss->nl_mgmt == NULL)
    { return -1; }
...
/*porting*/
static int nl80211_mgmt_unreg_sock_hdl(struct nl_msg *msg, void *arg)
{
    //nlmsg_free(msg);
    return 0;
}
static void nl80211_mgmt_unregister_socket(struct i802_bss *bss, struct nl_handle *handle)
{
    if (handle) {
        struct nl_msg *msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_UNREG_SOCKET);
        if (msg) {
            send_and_recv(bss->drv->global, handle, msg, nl80211_mgmt_unreg_sock_hdl, 0, 0, 0);
        }
    }
}
...
static void nl80211_mgmt_unsubscribe(struct i802_bss *bss, const char *reason)
{
    ...
    nl80211_mgmt_unregister_socket(bss, bss->nl_mgmt);//porting
...
static int nl80211_set_p2pdev(struct i802_bss *bss, int start)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: %s P2P Device %s (0x%llx): %s",
               ...
               bss->ifname, (long long unsigned int) bss->wdev_id,
               strerror(-ret));
    PRINTF("%s:%s P2P Device %s (0x%llx): RET:%d,%s",
           __FUNCTION__,
           start ? "Start" : "Stop",
           bss->ifname, (long long unsigned int) bss->wdev_id,
           ret, strerror(-ret));
...
*/
    ...
    nl80211_put_wiphy_data_ap(bss);//porting
    return ret;
}
...
static void wpa_driver_nl80211_deinit(struct i802_bss *bss)
{
    ...
    PRINTF("%s,%d:Ungister read socket:%p\n", __FUNCTION__, __LINE__, drv->eapol_sock);
    if (drv->eapol_sock && drv->eapol_sock != -1) { /*porting*/
        eloop_unregister_read_sock(drv->eapol_sock);
        nl_destroy_handles((struct nl_handle **)(&drv->eapol_sock));//porting
    }
#if 0//porting
                            ...
                            struct i802_bss *bss)
{
    ...
    if ((int)nl_connect == 0 || (int)nl_connect == -1) {
        PRINTF("%s,%d: invaild handle:%d\r\n", __FUNCTION__, __LINE__, nl_connect);
        return -1;
    }
...
#ifdef CONFIG_MAC80211_IBSS //porting
    if (drv->nlmode == NL80211_IFTYPE_ADHOC) {
        nl80211_mark_disconnected(drv);
    ...
    if (ret) {
        ...
        if (ret == -ENOENT && params->freq && !is_retry) {
            ...
            os_memcpy(event.timeout_event.addr, drv->auth_bssid_,
                      ETH_ALEN);
            drv->event(drv->ctx, EVENT_AUTH_TIMED_OUT,//porting
                       &event);
        }
    } else {
        wpa_printf(MSG_DEBUG,
                                      ...
                                      struct hostapd_acl_params *params)
{
    ...
    acl = nlmsg_new(acl_nlmsg_sz, 0);//porting
    if (!acl)
    { return -ENOMEM; }
    ...
    if (!(msg = nl80211_ifindex_msg_build(drv, nlmsg_new(nlmsg_sz, 0),//porting
                                          drv->ifindex, 0,
...
fail:
        ...
        PRINTF("Failed to create interface %s: %d (%s)",
               ifname, ret, strerror(-ret));
        return ret;
    }
    ...
    size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
    ...
    //porting
    res = wpa_driver_nl80211_send_mlme(bss, (u8 *) hdr, len, 0, 0,
                                       0, 0, 0, 0, 0, 0);
    if (res < 0) {
        wpa_printf(MSG_ERROR,
                                 ...
                                 struct wpa_driver_associate_params *params)
{
    ...
    if (wpa_driver_nl80211_set_mode(drv->first_bss, nlmode)) {
        //nl80211_remove_monitor_interface(drv);//porting
        return -1;
    }
    ...
    void *priv, struct wpa_driver_associate_params *params)
{
    ...
    if (ret) {
        wpa_dbg(drv->ctx, MSG_DEBUG,
                ...
                ret, strerror(-ret));
        //wpa_driver_nl80211_dump_scan(drv);//porting
    } else {
        wpa_printf(MSG_DEBUG,
...
static int wpa_driver_nl80211_set_operstate(void *priv, int state)
{
    ...
    wpa_printf(MSG_DEBUG, "nl80211: Set %s operstate %d->%d (%s)",
               ...
               state ? "UP" : "DORMANT");
    PRINTF("nl80211: Set %s operstate %d->%d (%s)\n",
           bss->ifname, drv->operstate, state,
           state ? "UP" : "DORMANT");
    drv->operstate = state;
    //porting
    return 0 ;//netlink_send_oper_ifla(drv->global->netlink, drv->ifindex, -1,
...
static int wpa_driver_nl80211_set_supp_port(void *priv, int authorized)
{
    ...
    if (!drv->associated && is_zero_ether_addr(drv->bssid) && !authorized) {
        ...
        //PRINTF("nl80211: Skip set_supp_port(unauthorized) while not associated\n");
        return 0;
    }
                            ...
                            const char *bridge_ifname, char *ifname_wds)
{
    ...
    if (val) {
        ...
        drv->event(bss->ctx, EVENT_WDS_STA_INTERFACE_STATUS,//porting
                   &event);
        return 0;
    }
}
...
static void handle_eapol(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    skb = nl_socket_recv(drv->eapol_sock, 100);
    if (skb == NULL) {
        wpa_printf(MSG_ERROR, "rx eapol failed!\r\n");
        return;
    }

    wpa_printf(MSG_MSGDUMP, "handle_eapol: rx eapol data %d bytes", skb->len);
    hdr = (struct ethhdr *)skb->data;
    drv_event_eapol_rx(drv->ctx, hdr->h_source, skb->data + 14, skb->len - 14, drv->hostapd);
    nlmsg_free(skb);
#else
    struct wpa_driver_nl80211_data *drv = eloop_ctx;
...
#if 1 /*porting*/
    drv->eapol_sock = (int)nl_create_handle((char *)params->ifname);
    if (drv->eapol_sock == (int)NULL) {
        wpa_printf(MSG_ERROR, "nl80211: socket(PF_PACKET, SOCK_DGRAM, ETH_P_PAE) failed: %s",
                   "error");
        goto failed;
    }
    i = nl_get_multicast_id("eapol");
    if (i >= 0) {
        if ((int)drv->eapol_sock == 0 || (int)drv->eapol_sock == -1) {
            PRINTF("%s,%d: invaild handle:%d\r\n", __FUNCTION__, __LINE__, (int)drv->eapol_sock);
            return -1;
        }
        nl_socket_add_membership((struct nl_sock *)drv->eapol_sock, i);
    }
#else
    ...
    enum wpa_driver_if_type type)
{
    switch (type) {
        ...
        default:
            return NL80211_IFTYPE_UNSPECIFIED;//porting
    }
}
                                  ...
                                  size_t csa_offs_len)
{
    ...
    cookie = 0;
    ret = send_and_recv_msgs(drv, msg, (no_ack ? NULL : cookie_handler), &cookie, NULL, NULL); //porting
    msg = NULL;
    if (ret) {
...
static void nl80211_frame_wait_cancel(struct i802_bss *bss, u64 cookie)
{
    ...
    if (!(msg = nl80211_cmd_msg(bss, 0, NL80211_CMD_FRAME_WAIT_CANCEL)) ||
        ...
        PRINTF("%s:NL80211_CMD_FRAME_WAIT_CANCEL ERROR!\n", __FUNCTION__);
        nlmsg_free(msg);
        return;
    }
...
static int wpa_driver_nl80211_cancel_remain_on_channel(void *priv)
{
    ...
    PRINTF("nl80211: Cancel remain-on-channel with cookie "
           "0x%llx",
           (long long unsigned int) drv->remain_on_chan_cookie);
    ...
    PRINTF("%s:Failed to cancel remain-on-channel:%d,%s\n",
...
static int wpa_driver_nl80211_probe_req_report(struct i802_bss *bss, int report)
{
    ...
    if (!report) {
        if (bss->nl_preq && drv->device_ap_sme &&
            ...
            PRINTF("%s:Disable nl_preq:%p\n", __FUNCTION__, bss->nl_preq);
            nl80211_mgmt_unregister_socket(bss, bss->nl_preq);
    ...
    bss->nl_preq = nl_create_handle(bss->ifname); /*porting*/
    if (bss->nl_preq == NULL)
    { return -1; }
...
static void *nl80211_global_init(void *ctx, char *ifname)  /*porting*/
{
    struct nl80211_global *global;
    //struct netlink_config *cfg;//porting

    global = os_zalloc(sizeof(*global));
    if (global == NULL)
    { return NULL; }
    global->ctx = ctx;
    global->ifname = ifname; /*porting*/
    global->ioctl_sock = -1;
...
static void nl80211_global_deinit(void *priv)
{
    ...
    if (!dl_list_empty(&global->interfaces)) {
        ...
        PRINTF("%s,%d: %u interface(s) remain at "
                   "nl80211_global_deinit",__FUNCTION__,__LINE__,dl_list_len(&global->interfaces));
    }

    //if (global->netlink)          //porting
    ...
    //if (global->ioctl_sock >= 0)          //porting
...
static int wpa_driver_nl80211_get_survey(void *priv, unsigned int freq)
{
    ...
    { drv->event(drv->ctx, EVENT_SURVEY, &data); }//porting
...
static const u8 *wpa_driver_nl80211_get_macaddr(void *priv)
{
    ...
    /*porting*/
    linux_get_ifhwaddr(drv->global->ioctl_sock, bss->ifname, bss->addr);
    memcpy(drv->perm_addr, bss->addr, ETH_ALEN);
...
static int vendor_reply_handler(struct nl_msg *msg, void *arg)
{
    ...
    struct genlmsghdr *gnlh = (struct genlmsghdr *)nlmsg_data(nlmsg_hdr(msg));//porting
    struct wpabuf *buf = (struct wpabuf *)arg;//porting
    int rem;
                                  ...
                                  int val)
{
    ...
    PRINTF("%s:nl80211: %s 4addr mode (bridge_ifname: %s)", __FUNCTION__,
           val ? "Enable" : "Disable", bridge_ifname);
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
    ...
    //.set_operstate = wpa_driver_nl80211_set_operstate,   //porting
    ...
    //.tx_control_port = nl80211_tx_control_port,  /*porting*/
    ...
    //.set_sta_vlan = driver_nl80211_set_sta_vlan,   //porting
