...
#if 0
static unsigned char p2p_inv_remote_addr[6] = {0};
                         

typedef void (*hgic_p2p_event_cb)(void *drv_priv, unsigned int event, unsigned int param);
struct hgic_p2p_event {
    struct mutex *mutex;
    void *drv_priv;
    hgic_p2p_event_cb callback;
};


int p2p_register_event_callback(void *priv,hgic_p2p_event_cb cb)
{
    
}

int p2p_event_proc(unsigned int event,unsigned int param)
{
    
}

void p2p_get_invitation_macaddr(unsigned char *mac) 
{
    os_mutex_lock(&p2p_inv_info_mutex);
    if(mac) {
        os_memcpy(mac,p2p_inv_remote_addr,6);
    }   
    os_mutex_unlock(&p2p_inv_info_mutex);
}                  

void p2p_reset_invitation_info(void)
{
    os_mutex_lock(&p2p_inv_info_mutex);
    memset(p2p_inv_remote_addr,0,6);
    os_mutex_unlock(&p2p_inv_info_mutex);
}

int p2p_invitation_info_init(void)
{
    memset(&p2p_inv_info_mutex,0,sizeof(struct mutex));
    memset(p2p_inv_remote_addr,0,6);
    os_mutex_init(&p2p_inv_info_mutex);
