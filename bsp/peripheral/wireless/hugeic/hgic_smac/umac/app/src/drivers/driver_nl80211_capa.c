...
{
	u32 *feat = arg;
    struct nlattr **tb_msg = NULL;//porting
					...
					struct nlattr *nl_akms)
{
	...
	nla_parse(tb, NL80211_IFTYPE_AKM_ATTR_MAX,
		  nla_data(nl_akms), nla_len(nl_akms), NULL, NULL);//porting
...
#if 1 /*porting: not support tx control port*/
	if (ext_feature_isset(ext_features, len,
...
{
	int rem = 0, i;
	struct nlattr *attr;//porting
    struct nlattr **tb1 = NULL;

	if (!tb || drv->num_iface_ext_capa == NL80211_IFTYPE_MAX)
		return;

    tb1 = MALLOC(sizeof(int)*(NL80211_ATTR_MAX + 1));//porting
    if(tb1 == NULL)
        return;
	...
	nla_for_each_nested(attr, tb, rem) {
		...
		nla_parse(tb1, NL80211_ATTR_MAX, nla_data(attr),
			  nla_len(attr), NULL, NULL);//porting
				       ...
				       struct wiphy_info_data *info)
{
	...
	else if (!info->connect_supported/* && !drv->hostapd*/) { /*porting: ap mode needn't connect_supported*/
			   ...
			   struct nlattr *nl_iftype)
{
	...
	nla_parse(tb, NL80211_BAND_IFTYPE_ATTR_MAX,
		  nla_data(nl_iftype), nla_len(nl_iftype), NULL, NULL);//porting

	if (!tb[NL80211_BAND_IFTYPE_ATTR_IFTYPES])
		return NL_STOP;

	if (nla_parse_nested(tb_flags, NL80211_IFTYPE_MAX,
			     tb[NL80211_BAND_IFTYPE_ATTR_IFTYPES], NULL, NULL))//porting
		return NL_STOP;
...
static int phy_info_band(struct phy_info_arg *phy_info, struct nlattr *nl_band)
{
	...
	nla_parse(tb_band, NL80211_BAND_ATTR_MAX, nla_data(nl_band),
		  nla_len(nl_band), NULL, NULL);//porting
				     ...
				     u16 *num_modes)
{
    ...
    PRINTF("nl80211: Added 802.11b mode based on 802.11g information\n");
...
static int nl80211_get_reg(struct nl_msg *msg, void *arg)
{
	...
	nla_for_each_nested(nl_rule, tb_msg[NL80211_ATTR_REG_RULES], rem_rule)
	{
		...
		nla_parse(tb_rule, NL80211_FREQUENCY_ATTR_MAX,
			  nla_data(nl_rule), nla_len(nl_rule), NULL, NULL);//porting
