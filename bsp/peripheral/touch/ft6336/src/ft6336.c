...
static rt_size_t ft6336_read_point(struct rt_touch_device *touch, void *buf, rt_size_t read_num) {
    ...
    if (ft6336_read_reg(&g_ft6336_client, &reg, &touch_status, 1) != RT_EOK) {
        LOG_E("read touch status failed");
        read_num = 0;
    ...
    if (touch_num) {
        ...
        for (rt_uint8_t i = 0; i < FT6336_MAX_TOUCH; i++) {
            read_id = (read_buf[i * 6 + 2] & 0xf0) >> 4;
            ft6336_touch_up(buf, read_id);
        }
    }
    ...
    if (touch_num) {
        ...
        point_state = (read_buf[0] & 0xf0) >> 6;  //1:up 2:down
        input_x = ((read_buf[0] & 0xf) << 8) | read_buf[1];
        input_y = ((read_buf[2] & 0xf) << 8) | read_buf[3];
